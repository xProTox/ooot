 ...
 */
static s32 BgCheck_PosErrorCheck(Vec3f* pos, const char* file, s32 line) {
    if (pos->x >= BGCHECK_XYZ_ABSMAX || pos->x <= -BGCHECK_XYZ_ABSMAX || pos->y >= BGCHECK_XYZ_ABSMAX ||
        ...
        // "Position is invalid."
        osSyncPrintf("T_BGCheck_PosErrorCheck(): Position is invalid. pos (%f,%f,%f) file:%s line:%d\n", pos->x,
...
s16 CollisionPoly_GetMinY(CollisionPoly* poly, Vec3s* vtxList) {
    ...
    //! @bug Due to rounding errors, some polys with a slight slope have a y normal of 1.0f/-1.0f. As such, pthis
...
void CollisionPoly_GetVerticesByBgId(CollisionPoly* poly, s32 bgId, CollisionContext* colCtx, Vec3f* dest) {
    ...
    if (poly == NULL || bgId > BG_ACTOR_MAX || dest == NULL) {
        ...
        osSyncPrintf("T_Polygon_GetVertex_bg_ai(): Error %d %d %d Argument not appropriate. Processing terminated.\n",
...
void BgCheck_Allocate(CollisionContext* colCtx, GlobalContext* globalCtx, CollisionHeader* colHeader) {
    ...
    if (YREG(15) == 0x10 || YREG(15) == 0x20 || YREG(15) == 0x30 || YREG(15) == 0x40) {
        ...
        osSyncPrintf("/* BGCheck Spot Spot Size %dbyte */\n", 0xF000);
    ...
    colCtx->lookupTbl = (StaticLookup*)THA_AllocEndAlign(
        &globalCtx->state.tha,
    ...
    if (customNodeListMax > 0) {
        ...
        // pthis is a non-issue as long as sceneSubdivisionList.nodeListMax is -1
    ...
    osSyncPrintf("/*---Eventually BG usage size %dbyte---*/\n", memSize + lookupTblMemSize);
    osSyncPrintf(VT_RST);
...
CollisionHeader* BgCheck_GetCollisionHeader(CollisionContext* colCtx, s32 bgId) {
    ...
    if (!(colCtx->dyna.bgActorFlags[bgId] & 1)) {
        ...
        osSyncPrintf("T_BGCheck_getBGDataInfo(): bg_actor_index is not used. index=%d\n");
        osSyncPrintf(VT_RST);
        return NULL;
    }
    ...
    *outBgId = BGCHECK_SCENE;
    if (BgCheck_PosErrorCheck(posA, "../z_bgcheck.c", 5334) == true ||
        ...
        if (actor != NULL) {
            ...
            osSyncPrintf("pself_actor == NULL, culprit unknown\n");
        }
    }
 ...
 * Init SSNodeList
 */
void SSNodeList_Initialize(SSNodeList* pthis) {
    pthis->max = 0;
    pthis->count = 0;
    pthis->tbl = NULL;
    pthis->polyCheckTbl = NULL;
}
 ...
 */
SSNode* SSNodeList_GetNextNode(SSNodeList* pthis) {
    SSNode* result = &pthis->tbl[pthis->count];

    pthis->count++;
    ASSERT(pthis->count < pthis->max, "pthis->short_slist_node_last_index < pthis->short_slist_node_size", "../z_bgcheck.c",
           5998);
...
u16 SSNodeList_GetNextNodeIdx(SSNodeList* pthis) {
    u16 new_index = pthis->count++;

    ASSERT(new_index < pthis->max, "new_index < pthis->short_slist_node_size", "../z_bgcheck.c", 6021);
    return new_index;
}
...
s32 DynaPoly_SetBgActor(GlobalContext* globalCtx, DynaCollisionContext* dyna, Actor* actor,
    ...
    if (foundSlot == false) {
        osSyncPrintf(VT_FGCOL(RED));
        osSyncPrintf("DynaPolyInfo_setActor(): dynamic polygon No free index\n");
        osSyncPrintf(VT_RST);
        return BG_ACTOR_MAX;
    }
...
void DynaPoly_DeleteBgActor(GlobalContext* globalCtx, DynaCollisionContext* dyna, s32 bgId) {
    ...
    if (DynaPoly_IsBgIdBgActor(bgId) == false) {
        ...
        if (bgId == -1) {
            ...
            osSyncPrintf(
                "DynaPolyInfo_delReserve():The index that should have been deleted(? ) was(== -1), processing aborted.\n");
            osSyncPrintf(VT_RST);
            return;
        } else {
            osSyncPrintf(VT_FGCOL(RED));
...
void DynaPoly_ExpandSRT(GlobalContext* globalCtx, DynaCollisionContext* dyna, s32 bgId, s32* vtxStartIndex,
    ...
    if (!(dyna->polyListMax >= *polyStartIndex + pbgdata->numPolygons)) {
        ...
        osSyncPrintf("DynaPolyInfo_expandSRT():polygon over do not use if %d exceeds %d\n",
...
void DynaPoly_Setup(GlobalContext* globalCtx, DynaCollisionContext* dyna) {
    ...
    for (i = 0; i < BG_ACTOR_MAX; i++) {
        if (dyna->bgActorFlags[i] & 2) {
            // Initialize BgActor
            osSyncPrintf(VT_FGCOL(GREEN));
            osSyncPrintf("DynaPolyInfo_setup():delete index=%d\n", i);
            osSyncPrintf(VT_RST);
...
void CollisionHeader_GetVirtual(void* colHeader, CollisionHeader** dest) {
    *dest = (CollisionHeader*)SEGMENTED_TO_VIRTUAL(colHeader);
...
u16 SurfaceType_GetCameraSType(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    ...
    camData = colHeader->cameraDataList;
    if (camData == (CamData*)gSegments[0]) {
        return 0;
    }
    surfaceTypes = colHeader->surfaceTypeList;
    if (surfaceTypes == (SurfaceType*)gSegments[0]) {
...
Vec3s* SurfaceType_GetCameraPositionData(CollisionContext* colCtx, s32 camId, s32 bgId) {
    ...
    if (cameraDataList == (CamData*)gSegments[0]) {
        return NULL;
    }
...
s32 WaterBox_GetSurfaceImpl(GlobalContext* globalCtx, CollisionContext* colCtx, f32 x, f32 z, f32* ySurface,
    ...
    if (colHeader->numWaterBoxes == 0 || colHeader->waterBoxes == (WaterBox*)gSegments[0]) {
        return false;
    }
