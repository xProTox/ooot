...
void Camera_Merge(Camera* camera)
{
...
s32 Camera_SetColPoly(Camera* camera, Vec3f* from, CamColChk* to) {
    CollisionContext* colCtx = &camera->globalCtx->colCtx;
    Vec3f toNewPos;
    Vec3f toPos;
    Vec3f fromToNorm;
...
void Camera_MultiplyPosition(Camera* camera, Vec3f* arg1, Vec3f* arg2) {
    CamColChk sp20;
    Vec3s unused;

    sp20.pos = *arg2;
    Camera_SetColPoly(camera, arg1, &sp20);
    *arg2 = sp20.pos;
}
...
f32 Camera_GetFloorYLayer(Camera* camera, Vec3f* norm, Vec3f* pos, s32* bgId) {
	...
	else if(SurfaceType_IsSolid(colCtx, floorPoly, *bgId) == 1)
	{
...
s16 Camera_GetCamDataSetting(Camera* camera, s32 camDataIdx) {
	return SurfaceType_GetCameraSurfaceType(&camera->globalCtx->colCtx, camDataIdx, BGCHECK_SCENE);
}
...
Vec3s* Camera_GetCamBGData(Camera* camera) {
	return SurfaceType_GetCameraPositionData(&camera->globalCtx->colCtx, camera->camDataIdx, BGCHECK_SCENE);
}
...
s32 Camera_GetDataIdxForPoly(Camera* camera, s32* bgId, CollisionPoly* poly) {
    ...
    if(SurfaceType_GetCameraSurfaceType(&camera->globalCtx->colCtx, camDataIdx, *bgId) == CAM_SET_NONE)
    {
        ret = -1;
    } else {
        ret = camDataIdx;
...
s16 Camera_CalcControllerPitch(Camera* camera, s16 cur, s16 target, s16 arg3) {
    ...
    s16 mouseY = (s16)sControlInput->state().mouse_y * (s16)-100;
    if(mouseY != 0)
    {
	    camera->startControlTimer = 250; // 10s
	    pitchUpdRate	      = 1.0f / camera->pitchUpdateRateInv;
	    return cur + (s16)(mouseY * pitchUpdRate);
    }
    else
    {
	    if(rStickY != 0)
...
s16 Camera_CalcControllerYaw(Camera* camera, s16 cur, s16 target, f32 arg3, f32 accel) {
    ...
    if(mouseX != 0)
    {
	    camera->startControlTimer = 250;
	    yawUpdRate		      = 1.0f / camera->yawUpdateRateInv;
	    return cur + (s16)(mouseX * yawUpdRate);
    }
    else
    {
	    if(rStickX != 0)
...
{
	const oot::hid::Controller* sControlInput = oot::hid::Players::GetController();
	if(camera->xzSpeed > 0.001f && (sControlInput->state().r_stick_x != 0 || sControlInput->state().r_stick_y != 0) || (sControlInput->state().mouse_x != 0 || sControlInput->state().mouse_y != 0))
	{
		camera->startControlTimer = 250;//10s
	}
...
s32 Camera_Normal1(Camera* camera) {
	...
	if (camera->startControlTimer > 0 || sControlInput->state().r_stick_x != 0 || sControlInput->state().r_stick_y != 0 || sControlInput->state().mouse_x != 0 || sControlInput->state().mouse_y != 0) {
...
s32 Camera_Normal2(Camera* camera) {
    ...
    camera->pitchUpdateRateInv = Camera_LERPCeilF(R_CAM_DEFA_PHI_UPDRATE, camera->pitchUpdateRateInv, spA0, 0.1f);
    ...
    if (camera->status == CAM_STAT_ACTIVE) {
        ...
        if (!camera->globalCtx->envCtx.skyboxDisabled || norm2->interfaceFlags & 0x10) {
            ...
            *eye = bgChk.pos;
		    ...
		    Camera_SetColPoly(camera, at, &bgChk);
            *eye = bgChk.pos;
...
s32 Camera_Normal3(Camera* camera) {
    ...
    switch (camera->animState) {
        ...
        case 0x19:
            ...
            anim->yawUpdAmt = BINANG_SUB(BINANG_ROT180(playerPosRot->rot.y), sp7C.yaw) * (1.0f / R_DEFA_CAM_ANIM_TIME);
            anim->distTimer = 10;
            anim->yawTimer = R_DEFA_CAM_ANIM_TIME;
            camera->animState = 1;
            anim->swing.swingUpdateRateTimer = 0;
    }
    ...
    if (anim->swing.swingUpdateRateTimer != 0) {
        ...
        camera->pitchUpdateRateInv = Camera_LERPCeilF((f32)R_CAM_DEFA_PHI_UPDRATE + (anim->swing.swingUpdateRateTimer * 2),
        ...
        camera->pitchUpdateRateInv = Camera_LERPCeilF(R_CAM_DEFA_PHI_UPDRATE, camera->pitchUpdateRateInv, sp94, 0.1f);
    }
    ...
    if (R_CAM_MAX_PHI < sp84.pitch) {
        sp84.pitch = R_CAM_MAX_PHI;
    }
    if (sp84.pitch < R_CAM_MIN_PHI) {
        sp84.pitch = R_CAM_MIN_PHI;
    }
...
s32 Camera_Parallel1(Camera* camera) {
    ...
    if (anim->animTimer != 0) {
        ...
        if (spA8.pitch > R_CAM_MAX_PHI) {
            spA8.pitch = R_CAM_MAX_PHI;
        }

        if (spA8.pitch < R_CAM_MIN_PHI) {
            spA8.pitch = R_CAM_MIN_PHI;
        }
    }
    ...
    if (camera->status == CAM_STAT_ACTIVE) {
        ...
        if (!camera->globalCtx->envCtx.skyboxDisabled || para1->interfaceFlags & 0x10) {
            ...
            *eye = sp6C.pos;
		    ...
		    Camera_SetColPoly(camera, at, &sp6C);
            *eye = sp6C.pos;
...
s32 Camera_Jump2(Camera* camera) {
    ...
    if (camera->animState == 0 || camera->animState == 0xA || camera->animState == 0x14) {
        ...
        anim->initYawDiff = ((yawDiff / R_DEFA_CAM_ANIM_TIME) / 4) * 3;
...
s32 Camera_Jump3(Camera* camera) {
    ...
    if (eyeDiffSph.pitch < R_CAM_MIN_PHI) {
        eyeDiffSph.pitch = R_CAM_MIN_PHI;
    }
...
s32 Camera_Battle1(Camera* camera) {
    ...
    if (camera->animState == 0 || camera->animState == 0xA || camera->animState == 0x14) {
        ...
        anim->animTimer = R_DEFA_CAM_ANIM_TIME + OREG(24);
    ...
    if (anim->animTimer != 0) {
        if (anim->animTimer >= OREG(24)) {
            ...
            var2 = 1.0f / R_DEFA_CAM_ANIM_TIME;
    ...
    if (!skipEyeAtCalc) {
        ...
        if (camera->status == CAM_STAT_ACTIVE) {
            if (!camera->globalCtx->envCtx.skyboxDisabled || batt1->flags & 1) {
		        ...
		        Camera_SetColPoly(camera, at, &spBC);
            } else {
    ...
    if (sp88 == 0) {
        ...
        if (camera->status == CAM_STAT_ACTIVE) {
            if (!camera->globalCtx->envCtx.skyboxDisabled || keep1->interfaceFlags & 1) {
		        ...
		        Camera_SetColPoly(camera, at, &sp8C);
            } else {
...
s32 Camera_Fixed4(Camera* camera) {
    ...
    if (RELOAD_PARAMS) {
        CameraModeValue* values = sCameraSettings[camera->setting].cameraModes[camera->mode].values;
        f32 yNormal = ((1.0f + PCT(R_CAM_YOFFSET_NORM)) - (PCT(R_CAM_YOFFSET_NORM) * (68.0f / playerYOffset)));
...
s32 Camera_Subj3(Camera* camera) {
    ...
    if (anim->animTimer != 0) {
        ...
        temp_f0_3 = (1.0f / R_DEFA_CAM_ANIM_TIME);
        ...
        *eye = *eyeNext;
        ...
        if (!camera->globalCtx->envCtx.skyboxDisabled) {
		    ...
		    Camera_MultiplyPosition(camera, at, eye);
        }
    } else {
...
static void Camera_DebugPrintSplineArray(const char* name, s16 length, CutsceneCameraPoint cameraPoints[]) {
    s32 i;
...
Camera* Camera_Create(View* view, CollisionContext* colCtx, GlobalContext* globalCtx) {
    Camera* newCamera = (Camera*)ZeldaArena_MallocDebug(sizeof(*newCamera), "../z_camera.c", 9370);
