...
s32 Player_InBlockingCsMode(GlobalContext* globalCtx, Player* pthis) {
    return (pthis->stateFlags1 & 0x20000080) || (pthis->csMode != 0) || (globalCtx->sceneLoadFlag == 0x14) ||
           (pthis->stateFlags1 & 1) || (pthis->stateFlags3 & 0x80) ||
...
void Player_SetModelsForHoldingShield(Player* pthis) {
    if ((pthis->stateFlags1 & 0x400000) &&
        ((pthis->itemActionParam < 0) || (pthis->itemActionParam == pthis->heldItemActionParam))) {
...
void func_8008EE08(Player* pthis) {
    if ((pthis->actor.bgCheckFlags & 1) || (pthis->stateFlags1 & 0x8A00000) ||
        (!(pthis->stateFlags1 & 0xC0000) && ((pthis->actor.world.pos.y - pthis->actor.floorHeight) < 100.0f))) {
        pthis->stateFlags1 &= ~0x400F8000;
    } else if (!(pthis->stateFlags1 & 0x2C0000)) {
    ...
    if (!Player_InCsMode(globalCtx)) {
        ...
        if ((triggerEntry->flag != 0) && !(gSaveContext.textTriggerFlags & triggerEntry->flag) &&
            (((var == 0) && (pthis->currentTunic != PLAYER_TUNIC_GORON)) ||
             (((var == 1) || (var == 3)) && (pthis->currentBoots == PLAYER_BOOTS_IRON) &&
              (pthis->currentTunic != PLAYER_TUNIC_ZORA)))) {
...
void func_8008F87C(GlobalContext* globalCtx, Player* pthis, SkelAnime* skelAnime, Vec3f* pos, Vec3s* rot,
    ...
    if ((pthis->actor.scale.y >= 0.0f) && !(pthis->stateFlags1 & 0x80) &&
        (Player_ActionToMagicSpell(pthis, pthis->itemActionParam) < 0)) {
        s32 pad;
...
s32 func_8008FCC8(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* pthisx) {
    ...
    if (limbIndex == PLAYER_LIMB_ROOT) {
        ...
        if (limbIndex == PLAYER_LIMB_HEAD) {
            ...
            func_8008F87C(globalCtx, pthis, &pthis->skelAnime, pos, rot, PLAYER_LIMB_R_THIGH, PLAYER_LIMB_R_SHIN,
                          PLAYER_LIMB_R_FOOT);
            return false;
        } else {
            return false;
        ...
        if (limbIndex == PLAYER_LIMB_L_HAND) {
            ...
            *dList = dLists[sDListsLodOffset];
            ...
            if ((pthis->sheathType == 18) || (pthis->sheathType == 19)) {
                dLists += pthis->currentShield * 4;
                if (!LINK_IS_ADULT && (pthis->currentShield < PLAYER_SHIELD_HYLIAN) &&
            ...
            } else if (!LINK_IS_ADULT && ((pthis->sheathType == 16) || (pthis->sheathType == 17)) &&
                       (gSaveContext.equips.buttonItems[0] != ITEM_SWORD_KOKIRI)) {
                dLists = D_80125D68;
            }
...
void func_800906D4(GlobalContext* globalCtx, Player* pthis, Vec3f* newTipPos) {
    ...
    if (func_80090480(globalCtx, NULL, &pthis->swordInfo[0], &newTipPos[0], &newBasePos[0]) &&
        !(pthis->stateFlags1 & 0x400000)) {
        EffectBlure_AddVertex((EffectBlure*)Effect_GetByIndex(pthis->swordEffectIndex), &pthis->swordInfo[0].tip,
                              &pthis->swordInfo[0].base);
    }

    if ((pthis->swordState > 0) && ((pthis->swordAnimation < 0x18) || (pthis->stateFlags2 & 0x20000))) {
...
void Player_DrawGetItemImpl(GlobalContext* globalCtx, Player* pthis, Vec3f* refPos, s32 drawIdPlusOne) {
    ...
    Matrix_Translate(refPos->x + (3.3f * Math_SinS(pthis->actor.shape.rot.y)), refPos->y + height,
                     refPos->z + ((3.3f + (IREG(90) / 10.0f)) * Math_CosS(pthis->actor.shape.rot.y)), MTXMODE_NEW);
...
void func_80090D20(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* pthisx) {
    ...
    if (limbIndex == PLAYER_LIMB_L_HAND) {
        ...
        if (pthis->rightHandType == 0xFF) {
            ...
            if ((pthis->stateFlags1 & 0x200) && (pthis->unk_860 >= 0) && (pthis->unk_834 <= 10)) {
                Vec3f sp90;
                f32 distXYZ;
        ...
        if (pthis->actor.scale.y >= 0.0f) {
            if ((pthis->heldItemActionParam == PLAYER_AP_HOOKSHOT) ||
                (pthis->heldItemActionParam == PLAYER_AP_LONGSHOT)) {
                ...
                if (heldActor != NULL) {
                    ...
                    if (func_8002DD78(pthis) != 0) {
                        ...
                        Player_DrawHookshotReticle(
                            globalCtx, pthis, (pthis->heldItemActionParam == PLAYER_AP_HOOKSHOT) ? 38600.0f : 77600.0f);
                    }
                }
            }

            if ((pthis->unk_862 != 0) || ((func_8002DD6C(pthis) == 0) && (heldActor != NULL))) {
                if (!(pthis->stateFlags1 & 0x400) && (pthis->unk_862 != 0) && (pthis->exchangeItemId != EXCH_ITEM_NONE)) {
