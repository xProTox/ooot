...
void Actor_SetObjectDependency(GlobalContext* globalCtx, Actor* actor) {
    gSegments[6] = gObjectTable[actor->objBankIndex].vromStart.get();
}
...
void Actor_Destroy(Actor* actor, GlobalContext* globalCtx) {
    ...
    if (actor->destroy != NULL) {
        ...
        osSyncPrintf("No Actor class destruct [%s]\n" VT_RST, name);
    }
}
...
void Actor_FaultPrint(Actor* actor, char* command) {
    ...
    osSyncPrintf("Actor name (%08x:%s)", actor, name); // "Actor name (%08x:%s)"

    if (command != NULL) {
        osSyncPrintf("Command:%s\n", command); // "Command:%s"
    }
...
void Actor_Draw(GlobalContext* globalCtx, Actor* actor) {
    ...
    gSPSegment(POLY_OPA_DISP++, 0x06, gObjectTable[actor->objBankIndex].vromStart.get());
    gSPSegment(POLY_XLU_DISP++, 0x06, gObjectTable[actor->objBankIndex].vromStart.get());
...
void func_8003115C(GlobalContext* globalCtx, s32 numInvisibleActors, Actor** invisibleActors) {
    ...
    gDPNoOpString(POLY_OPA_DISP++, "Magic lens START", 0); // "Magic lens START"
    ...
    gDPNoOpString(POLY_OPA_DISP++, "Magic lens invisible Actor display START", numInvisibleActors);

    invisibleActor = &invisibleActors[0];
    for (i = 0; i < numInvisibleActors; i++) {
        // "Magic lens invisible Actor display"
        gDPNoOpString(POLY_OPA_DISP++, "Magic lens invisible Actor display", i);
        Actor_Draw(globalCtx, *(invisibleActor++));
    }

    // "Magic lens invisible Actor display END"
    gDPNoOpString(POLY_OPA_DISP++, "Magic lens invisible Actor display END", numInvisibleActors);

    if (globalCtx->roomCtx.curRoom.showInvisActors != 0) {
        gDPNoOpString(POLY_OPA_DISP++, "Blue spectacles (exterior)", 0); // "Blue spectacles (exterior)"
        ...
        gDPNoOpString(POLY_OPA_DISP++, "Blue spectacles (exterior)", 1); // "Blue spectacles (exterior)"
    }

    gDPNoOpString(POLY_OPA_DISP++, "Magic lens END", 0); // "Magic lens END"
...
void func_800315AC(GlobalContext* globalCtx, ActorContext* actorCtx) {
    ...
    for (i = 0; i < ARRAY_COUNT(actorCtx->actorLists); i++, actorListEntry++) {
        ...
        while (actor != NULL) {
            ...
            const char* actorName = overlayEntry->name != NULL ? overlayEntry->name : "";
...
void func_80031C3C(ActorContext* actorCtx, GlobalContext* globalCtx) {
    ...
    if (HREG(20) != 0) {
        osSyncPrintf("Absolute magic field deallocation\n"); // "Absolute magic field deallocation"
    }
...
void Actor_FreeOverlay(ActorOverlay* actorOverlay) {
    ...
    if (actorOverlay->numLoaded == 0) {
        if (HREG(20) != 0) {
            osSyncPrintf("Actor client is now 0\n"); // "Actor client is now 0"
        }

        if (actorOverlay->loadedRamAddr.buffer() != NULL) {
            if (actorOverlay->allocType & ALLOCTYPE_PERMANENT) {
                if (HREG(20) != 0) {
                    osSyncPrintf("Overlay will not be deallocated\n"); // "Overlay will not be deallocated"
                }
            } else if (actorOverlay->allocType & ALLOCTYPE_ABSOLUTE) {
                if (HREG(20) != 0) {
                    // "Absolute magic field reserved, so deallocation will not occur"
                    osSyncPrintf("Absolute magic field reserved, so deallocation will not occur\n");
                }
                actorOverlay->loadedRamAddr = NULL;
            } else {
                if (HREG(20) != 0) {
                    osSyncPrintf("Overlay deallocated\n"); // "Overlay deallocated"
                }
                ZeldaArena_FreeDebug(actorOverlay->loadedRamAddr.buffer(), "../z_actor.c", 6834);
                actorOverlay->loadedRamAddr = NULL;
            }
        }
    } else if (HREG(20) != 0) {
        // "%d of actor client remains"
        osSyncPrintf("%d of actor client remains\n", actorOverlay->numLoaded);
    }
...
Actor* Actor_Spawn(ActorContext* actorCtx, GlobalContext* globalCtx, s16 actorId, f32 posX, f32 posY, f32 posZ,
    ...
    if (overlayEntry->vramStart.buffer() == 0) {
        if (HREG(20) != 0) {
        ...
        if (overlayEntry->loadedRamAddr.buffer() != NULL) {
            if (HREG(20) != 0) {
            ...
            if (overlayEntry->allocType & ALLOCTYPE_ABSOLUTE) {
                ...
                if (actorCtx->absoluteSpace == NULL) {
                    ...
                    actorCtx->absoluteSpace = ZeldaArena_MallocRDebug(AM_FIELD_SIZE, "AMF: absolute magic field", 0);
                    if (HREG(20) != 0) {
...
void BodyBreak_Alloc(BodyBreak* bodyBreak, s32 count, GlobalContext* globalCtx) {
    ...
    bodyBreak->matrices = (MtxF*)ZeldaArena_MallocDebug(matricesSize, "../z_actor.c", 7540);

    if (bodyBreak->matrices != NULL) {
        dListsSize = (count + 1) * sizeof(*bodyBreak->dLists);
        bodyBreak->dLists = (Gfx**)ZeldaArena_MallocDebug(dListsSize, "../z_actor.c", 7543);

        if (bodyBreak->dLists != NULL) {
            objectIdsSize = (count + 1) * sizeof(*bodyBreak->objectIds);
            bodyBreak->objectIds = (s16*)ZeldaArena_MallocDebug(objectIdsSize, "../z_actor.c", 7546);
...
Gfx* func_80034B54(GraphicsContext* gfxCtx) {
    ...
    displayList = displayListHead = (Gfx*)Graph_Alloc(gfxCtx, 2 * sizeof(Gfx));
...
Actor* func_800358DC(Actor* actor, Vec3f* spawnPos, Vec3s* spawnRot, f32* arg3, s32 timer, s16* unused,
    ...
    if (spawnedEnPart != NULL) {
        ...
        spawnedEnPart->displayList = (Gfx*)arg8;
