...
void GetItem_DrawMirrorShield(GlobalContext* globalCtx, s16 drawId) {
    ...
    gSPSegment(POLY_OPA_DISP++, 0x08,
               Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, Counter(globalCtx->state.frames * 0) % 256,
                                Counter(globalCtx->state.frames * 2) % 256, 64, 64, 1,
                                Counter(globalCtx->state.frames * 0) % 128, Counter(globalCtx->state.frames * 1) % 128, 32,
                                32));
...
void GetItem_DrawScale(GlobalContext* globalCtx, s16 drawId) {
    ...
    gSPSegment(POLY_XLU_DISP++, 0x08,
               Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 1 * (globalCtx->state.frames * 2),
                                (globalCtx->state.frames * -2), 64, 64, 1, 1 * (globalCtx->state.frames * 4),
                                (globalCtx->state.frames * -4), 32, 32));
