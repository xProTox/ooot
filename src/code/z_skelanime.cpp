...
void SkelAnime_DrawFlexLod(GlobalContext* globalCtx, void** skeleton, Vec3s* jointTable, s32 dListCount,
    ...
    Vec3f pos;
    Vec3s rot;
    Mtx* mtx = (Mtx*)Graph_Alloc(globalCtx->state.gfxCtx, dListCount * sizeof(Mtx));
...
void SkelAnime_GetFrameData(AnimationHeader* animation, s32 frame, s32 limbCount, Vec3s* frameTable) {
    ...
    JointIndex* jointIndices = (JointIndex*)SEGMENTED_TO_VIRTUAL(animHeader->jointIndices);
    s16* frameData = (s16*)SEGMENTED_TO_VIRTUAL(animHeader->frameData);
...
s16 Animation_GetLength(void* animation) {
    AnimationHeaderCommon* common = (AnimationHeaderCommon*)SEGMENTED_TO_VIRTUAL(animation);
...
s32 SkelAnime_GetFrameDataLegacy(LegacyAnimationHeader* animation, s32 frame, Vec3s* frameTable) {
    LegacyAnimationHeader* animHeader = (LegacyAnimationHeader*)SEGMENTED_TO_VIRTUAL(animation);
    s32 limbCount = animHeader->limbCount;
    JointKey* key = (JointKey*)SEGMENTED_TO_VIRTUAL(animHeader->jointKey);
    s16* frameData = (s16*)SEGMENTED_TO_VIRTUAL(animHeader->frameData);
    s16* staticData = &frameData[0];
    s16* dynamicData = &frameData[frame];
    s32 i;
 ...
 */
void AnimationContext_SetLoadFrame(GlobalContext* globalCtx, LinkAnimationHeader* animation, float frame, s32 limbCount,
                                   Vec3s* frameTable) {
    AnimationEntry* entry = AnimationContext_AddEntry(&globalCtx->animationCtx, ANIMENTRY_LOADFRAME);

    if (entry != NULL) {
	    LinkAnimationHeader* linkAnimHeader = (LinkAnimationHeader*)SEGMENTED_TO_VIRTUAL(animation);
        void* ram = frameTable;
	    s32 frame1				= (s32)frame;

        osCreateMesgQueue(&entry->data.load.msgQueue, &entry->data.load.msg, 1);
 
        DmaMgr_SendRequest2(&entry->data.load.req, ram,
                            LINK_ANIMATION_OFFSET(linkAnimHeader->segment, ((sizeof(Vec3s) * limbCount + 2) * frame1)),
...
void SkelAnime_InitLink(GlobalContext* globalCtx, SkelAnime* skelAnime, FlexSkeletonHeader* skeletonHeaderSeg,
                        ...
                        s32 limbBufCount) {
    FlexSkeletonHeader* skeletonHeader = (FlexSkeletonHeader*)SEGMENTED_TO_VIRTUAL(skeletonHeaderSeg);
    ...
    skelAnime->skeleton = (void**)SEGMENTED_TO_VIRTUAL(skeletonHeader->sh.segment);
    ...
    if (jointTable == NULL) {
        skelAnime->jointTable = (Vec3s*)ZeldaArena_MallocDebug(allocSize, "../z_skelanime.c", 2364);
        skelAnime->morphTable = (Vec3s*)ZeldaArena_MallocDebug(allocSize, "../z_skelanime.c", 2365);
    } else {
...
s32 SkelAnime_Init(GlobalContext* globalCtx, SkelAnime* skelAnime, SkeletonHeader* skeletonHeaderSeg,
    ...
    SkeletonHeader* skeletonHeader = (SkeletonHeader*)SEGMENTED_TO_VIRTUAL(skeletonHeaderSeg);

    skelAnime->limbCount = skeletonHeader->limbCount + 1;
    skelAnime->skeleton = (void**)SEGMENTED_TO_VIRTUAL(skeletonHeader->segment);
    if (jointTable == NULL) {
        skelAnime->jointTable = (Vec3s*)ZeldaArena_MallocDebug(skelAnime->limbCount * sizeof(*skelAnime->jointTable), "../z_skelanime.c", 2968);
        skelAnime->morphTable = (Vec3s*)ZeldaArena_MallocDebug(skelAnime->limbCount * sizeof(*skelAnime->morphTable), "../z_skelanime.c", 2969);
    } else {
...
s32 SkelAnime_InitFlex(GlobalContext* globalCtx, SkelAnime* skelAnime, FlexSkeletonHeader* skeletonHeaderSeg,
    ...
    if (jointTable == NULL) {
        skelAnime->jointTable = (Vec3s*)ZeldaArena_MallocDebug(skelAnime->limbCount * sizeof(*skelAnime->jointTable), "../z_skelanime.c", 3047);

        skelAnime->morphTable = (Vec3s*)ZeldaArena_MallocDebug(skelAnime->limbCount * sizeof(*skelAnime->morphTable), "../z_skelanime.c", 3048);
    } else {
...
s32 SkelAnime_InitSkin(GlobalContext* globalCtx, SkelAnime* skelAnime, SkeletonHeader* skeletonHeaderSeg,
    ...
    skelAnime->limbCount = skeletonHeader->limbCount + 1;
    skelAnime->skeleton = (void**)SEGMENTED_TO_VIRTUAL(skeletonHeader->segment);
    skelAnime->jointTable = (Vec3s*)ZeldaArena_MallocDebug(skelAnime->limbCount * sizeof(*skelAnime->jointTable), "../z_skelanime.c", 3120);
    skelAnime->morphTable = (Vec3s*)ZeldaArena_MallocDebug(skelAnime->limbCount * sizeof(*skelAnime->morphTable), "../z_skelanime.c", 3121);
    if ((skelAnime->jointTable == NULL) || (skelAnime->morphTable == NULL)) {
        osSyncPrintf(VT_FGCOL(RED));
        // "Memory allocation error"
...
s32 SkelAnime_Update(SkelAnime* skelAnime) {
    return skelAnime->update(nullptr, skelAnime);
}

/**
 * Morphs from the pose in jointTable to the one in morphTable, advancing the morph but not the animation frame
 */
static s32 SkelAnime_Morph(GlobalContext* context, SkelAnime* skelAnime)
{
    f32 prevMorphWeight = skelAnime->morphWeight;
    f32 updateRate = FMV2;
 ...
 * animation frame
 */
static s32 SkelAnime_MorphTaper(GlobalContext* context, SkelAnime* skelAnime)
{
 ...
 */
static s32 SkelAnime_Once(GlobalContext* context, SkelAnime* skelAnime) {
    f32 updateRate = FMV2;

    if (skelAnime->curFrame == skelAnime->endFrame) {
	    SkelAnime_GetFrameData((AnimationHeader*)skelAnime->animation, (s32)skelAnime->curFrame, skelAnime->limbCount,
                               skelAnime->jointTable);
        SkelAnime_AnimateFrame(skelAnime);
        return 1;
    }
