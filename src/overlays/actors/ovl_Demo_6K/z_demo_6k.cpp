...
void Demo6K_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Lights_PointNoGlowSetInfo(&pthis->lightInfo, pthis->actor.world.pos.x, pthis->actor.world.pos.y,
                              pthis->actor.world.pos.z, 255, 255, 255, 100);
    ...
    if (pthis->timer1 < 5) {
        ...
        if (pthis->timer1 == 15) {
            Actor_SpawnAsChild(&globalCtx->actorCtx, &pthis->actor, globalCtx, ACTOR_EFF_DUST, pthis->actor.world.pos.x,
                               pthis->actor.world.pos.y, pthis->actor.world.pos.z, 0, 0, 0, EFF_DUST_TYPE_1);
        }
...
void func_809674E0(Demo6K* pthis, GlobalContext* globalCtx) {
    ...
    Lights_PointNoGlowSetInfo(&pthis->lightInfo, pthis->actor.world.pos.x, pthis->actor.world.pos.y,
                              pthis->actor.world.pos.z, sEnvColors[pthis->unk_293].r, sEnvColors[pthis->unk_293].g,
                              sEnvColors[pthis->unk_293].b, pthis->actor.scale.x * 4000.0f);
}
...
void func_80967DBC(Demo6K* pthis, GlobalContext* globalCtx) {
    ...
    if (pthis->timer2 > 44) {
        ...
        Lights_PointNoGlowSetInfo(&pthis->lightInfo, pthis->actor.world.pos.x, pthis->actor.world.pos.y,
                                  pthis->actor.world.pos.z, 255, 200, 0, pthis->unk_293);
    }
}

void func_80967F10(Demo6K* pthis, GlobalContext* globalCtx) {
    if (pthis->timer2 == 0) {
        Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_DEMO_6K, pthis->actor.world.pos.x, pthis->actor.world.pos.y,
                    pthis->actor.world.pos.z, 0, 0, 0, 13);
    }
...
void func_8096865C(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (!(pthis->flags & 1)) {
        ...
        gDPSetEnvColor(POLY_XLU_DISP++, sEnvColors[pthis->unk_293].r, sEnvColors[pthis->unk_293].g,
                       sEnvColors[pthis->unk_293].b, 255);
...
void func_809688C4(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if ((i = (globalCtx->csCtx.state != CS_STATE_IDLE) && (globalCtx->csCtx.npcActions[1] != NULL)) &&
        ...
        for (i = 0; i < 16; i++) {
            gDPPipeSync(POLY_XLU_DISP++);
            gDPSetEnvColor(POLY_XLU_DISP++, sEnvColors[pthis->unk_274[i]].r, sEnvColors[pthis->unk_274[i]].g,
                           sEnvColors[pthis->unk_274[i]].b, 255);
            Matrix_Push();
            Matrix_Translate(pthis->unk_174[i], pthis->unk_1B4[i], 0.0f, MTXMODE_APPLY);
            Matrix_Scale(pthis->unk_234[i] * D_8096931C[s32(frames + i) & 3],
                         pthis->unk_234[i] * D_8096931C[s32(frames + i) & 3],
                         pthis->unk_234[i] * D_8096931C[s32(frames + i) & 3], MTXMODE_APPLY);
