...
EnGo
pathIndex: pthis->actor.params & 0xF
Goron: pthis->actor.params & 0xF0

EnGo2
(pthis->actor.params & 0x3E0) >> 5
(pthis->actor.params & 0xFC00) >> 0xA - Gorons in Fire Temple
this->actor.params & 0x1F

Gorons only move when pthis->unk_194.unk_00 == 0
*/
...
void EnGo2_SlowRolling(EnGo2* pthis, GlobalContext* globalCtx);
void EnGo2_GroundRolling(EnGo2* pthis, GlobalContext* globalCtx);

void EnGo2_ReverseRolling(EnGo2* pthis, GlobalContext* globalCtx);
void EnGo2_SetupGetItem(EnGo2* pthis, GlobalContext* globalCtx);
void EnGo2_SetGetItem(EnGo2* pthis, GlobalContext* globalCtx);
void EnGo2_BiggoronEyedrops(EnGo2* pthis, GlobalContext* globalCtx);
void EnGo2_GoronLinkStopRolling(EnGo2* pthis, GlobalContext* globalCtx);
void EnGo2_GoronFireGenericAction(EnGo2* pthis, GlobalContext* globalCtx);
...
void EnGo2_UpdateDust(EnGo2* pthis) {
    EnGoEffect* dustEffect = pthis->dustEffects;
    f32 randomNumber;
    s16 i;
...
void EnGo2_DrawDust(EnGo2* pthis, GlobalContext* globalCtx) {
    EnGoEffect* dustEffect = pthis->dustEffects;
    s16 alpha;
    s16 firstDone;
    s16 index;
...
s32 EnGo2_SpawnDust(EnGo2* pthis, u8 initialTimer, f32 scale, f32 scaleStep, s32 numDustEffects, f32 radius,
                    f32 yAccel) {
    Vec3f pos = sPos;
    ...
    pos = pthis->actor.world.pos; // overwrites sPos data
    ...
    while (i >= 0) {
        ...
        EnGo2_AddDust(pthis, &pos, &velocity, &accel, initialTimer, scale, scaleStep);
...
void EnGo2_GetItem(EnGo2* pthis, GlobalContext* globalCtx, s32 getItemId) {
    pthis->getItemId = getItemId;
    func_8002F434(&pthis->actor, globalCtx, getItemId, pthis->actor.xzDistToPlayer + 1.0f,
                  fabsf(pthis->actor.yDistToPlayer) + 1.0f);
}

s32 EnGo2_GetDialogState(EnGo2* pthis, GlobalContext* globalCtx) {
    s16 dialogState = Message_GetState(&globalCtx->msgCtx);

    if ((pthis->dialogState == TEXT_STATE_AWAITING_NEXT) || (pthis->dialogState == TEXT_STATE_EVENT) ||
        (pthis->dialogState == TEXT_STATE_CLOSING) || (pthis->dialogState == TEXT_STATE_DONE_HAS_NEXT)) {
        if (dialogState != pthis->dialogState) {
            pthis->unk_20C++;
        }
    }

    pthis->dialogState = dialogState;
    return dialogState;
}

u16 EnGo2_GoronFireGenericGetTextId(EnGo2* pthis) {
    switch ((pthis->actor.params & 0xFC00) >> 0xA) {
        case 3:
            return 0x3069;
        case 5:
            return 0x306A;
...
u16 EnGo2_GetTextIdGoronCityRollingBig(GlobalContext* globalCtx, EnGo2* pthis) {
    if (gSaveContext.infTable[17] & 0x4000) {
        return 0x3013;
    } else if (CUR_CAPACITY(UPG_BOMB_BAG) >= 20 && pthis->waypoint > 7 && pthis->waypoint < 12) {
        return 0x3012;
    } else {
        return 0x3011;
    }
...
s16 EnGo2_GetStateGoronCityRollingBig(GlobalContext* globalCtx, EnGo2* pthis) {
    s32 bombBagUpgrade;
    ...
    switch (Message_GetState(&globalCtx->msgCtx)) {
        ...
        case TEXT_STATE_EVENT:
            if (Message_ShouldAdvance(globalCtx)) {
                if (pthis->actor.textId == 0x3012) {
                    pthis->actionFunc = EnGo2_SetupGetItem;
                    bombBagUpgrade = CUR_CAPACITY(UPG_BOMB_BAG) == 30 ? GI_BOMB_BAG_40 : GI_BOMB_BAG_30;
                    EnGo2_GetItem(pthis, globalCtx, bombBagUpgrade);
...
u16 EnGo2_GetTextIdGoronDmtBombFlower(GlobalContext* globalCtx, EnGo2* pthis) {
    return CHECK_QUEST_ITEM(QUEST_GORON_RUBY) ? 0x3027 : 0x300A;
}

// DMT Goron by Bomb Flower Choice
s16 EnGo2_GetStateGoronDmtBombFlower(GlobalContext* globalCtx, EnGo2* pthis) {
    switch (Message_GetState(&globalCtx->msgCtx)) {
        case TEXT_STATE_CLOSING:
            if ((pthis->actor.textId == 0x300B) && (gSaveContext.infTable[14] & 0x800) == 0) {
        ...
        case TEXT_STATE_CHOICE:
            if (Message_ShouldAdvance(globalCtx)) {
                ...
                if (pthis->actor.textId == 0x300A) {
                    if (globalCtx->msgCtx.choiceIndex == 0) {
                        pthis->actor.textId = CUR_UPG_VALUE(UPG_STRENGTH) != 0 ? 0x300B : 0x300C;
                    } else {
                        pthis->actor.textId = 0x300D;
                    }
...
u16 EnGo2_GetTextIdGoronDmtRollingSmall(GlobalContext* globalCtx, EnGo2* pthis) {
...
s16 EnGo2_GetStateGoronDmtRollingSmall(GlobalContext* globalCtx, EnGo2* pthis) {
...
u16 EnGo2_GetTextIdGoronDmtDcEntrance(GlobalContext* globalCtx, EnGo2* pthis) {
...
s16 EnGo2_GetStateGoronDmtDcEntrance(GlobalContext* globalCtx, EnGo2* pthis) {
    if (Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_CLOSING) {
        if (pthis->actor.textId == 0x3008) {
...
u16 EnGo2_GetTextIdGoronCityEntrance(GlobalContext* globalCtx, EnGo2* pthis) {
    if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE) && LINK_IS_ADULT) {
        return 0x3043;
    } else if (CHECK_QUEST_ITEM(QUEST_GORON_RUBY)) {
        return 0x3027;
...
s16 EnGo2_GetStateGoronCityEntrance(GlobalContext* globalCtx, EnGo2* pthis) {
    if (Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_CLOSING) {
        if (pthis->actor.textId == 0x3014) {
...
u16 EnGo2_GetTextIdGoronCityIsland(GlobalContext* globalCtx, EnGo2* pthis) {
    if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE) && LINK_IS_ADULT) {
        return 0x3043;
    } else if (CHECK_QUEST_ITEM(QUEST_GORON_RUBY)) {
        return 0x3067;
...
s16 EnGo2_GetStateGoronCityIsland(GlobalContext* globalCtx, EnGo2* pthis) {
    if (Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_CLOSING) {
        if (pthis->actor.textId == 0x3016) {
...
u16 EnGo2_GetTextIdGoronCityLowestFloor(GlobalContext* globalCtx, EnGo2* pthis) {
    if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE) && LINK_IS_ADULT) {
        return 0x3043;
    } else if (CHECK_QUEST_ITEM(QUEST_GORON_RUBY)) {
        return 0x3027;
...
s16 EnGo2_GetStateGoronCityLowestFloor(GlobalContext* globalCtx, EnGo2* pthis) {
    if (Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_CLOSING) {
        if (pthis->actor.textId == 0x3018) {
...
u16 EnGo2_GetTextIdGoronCityLink(GlobalContext* globalCtx, EnGo2* pthis) {
    if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE)) {
        ...
        pthis->unk_20C = 0;
        pthis->dialogState = TEXT_STATE_NONE;
...
s16 EnGo2_GetStateGoronCityLink(GlobalContext* globalCtx, EnGo2* pthis) {
    switch (EnGo2_GetDialogState(pthis, globalCtx)) {
        case TEXT_STATE_CLOSING:
            switch (pthis->actor.textId) {
                case 0x3036:
                    EnGo2_GetItem(pthis, globalCtx, GI_TUNIC_GORON);
                    pthis->actionFunc = EnGo2_SetupGetItem;
                    return 2;
                case 0x3037:
        ...
        case TEXT_STATE_CHOICE:
            if (Message_ShouldAdvance(globalCtx)) {
                if (pthis->actor.textId == 0x3034) {
                    if (globalCtx->msgCtx.choiceIndex == 0) {
                        pthis->actor.textId = gSaveContext.infTable[16] & 0x800 ? 0x3033 : 0x3035;
                        if (pthis->actor.textId == 0x3035) {
                            Audio_StopSfxById(NA_SE_EN_GOLON_CRY);
                        }
                    } else {
                        pthis->actor.textId = gSaveContext.infTable[16] & 0x800 ? 0x3036 : 0x3033;
                        if (pthis->actor.textId == 0x3036) {
                            Audio_StopSfxById(NA_SE_EN_GOLON_CRY);
                        }
                    }
                    Message_ContinueTextbox(globalCtx, pthis->actor.textId);
...
u16 EnGo2_GetTextIdGoronDmtBiggoron(GlobalContext* globalCtx, EnGo2* pthis) {
...
s16 EnGo2_GetStateGoronDmtBiggoron(GlobalContext* globalCtx, EnGo2* pthis) {
    s32 unusedPad;
    u8 dialogState = pthis->dialogState;

    switch (EnGo2_GetDialogState(pthis, globalCtx)) {
        case TEXT_STATE_DONE:
            if (pthis->actor.textId == 0x305E) {
                if (!gSaveContext.bgsFlag) {
                    EnGo2_GetItem(pthis, globalCtx, GI_SWORD_BGS);
                    pthis->actionFunc = EnGo2_SetupGetItem;
                    return 2;
                } else {
                    return 0;
        ...
        case TEXT_STATE_CHOICE:
            if (Message_ShouldAdvance(globalCtx)) {
                if ((pthis->actor.textId == 0x3054) || (pthis->actor.textId == 0x3055)) {
                    if (globalCtx->msgCtx.choiceIndex == 0) {
                        EnGo2_GetItem(pthis, globalCtx, GI_PRESCRIPTION);
                        pthis->actionFunc = EnGo2_SetupGetItem;
                        return 2;
                    }
                    pthis->actor.textId = 0x3056;
                    Message_ContinueTextbox(globalCtx, pthis->actor.textId);
                }
                return 1;
            }
...
u16 EnGo2_GetTextIdGoronFireGeneric(GlobalContext* globalCtx, EnGo2* pthis) {
...
s16 EnGo2_GetStateGoronFireGeneric(GlobalContext* globalCtx, EnGo2* pthis) {
    switch (Message_GetState(&globalCtx->msgCtx)) {
        case TEXT_STATE_CLOSING:
            return 0;
        case TEXT_STATE_EVENT:
            if (Message_ShouldAdvance(globalCtx)) {
                if (pthis->actor.textId == 0x3071) {
                    pthis->actor.textId = EnGo2_GoronFireGenericGetTextId(pthis);
                    Message_ContinueTextbox(globalCtx, pthis->actor.textId);
                }
                return 1;
            }
...
u16 EnGo2_GetTextIdGoronCityStairwell(GlobalContext* globalCtx, EnGo2* pthis) {
    return !LINK_IS_ADULT ? gSaveContext.infTable[14] & 0x8 ? 0x3022 : 0x300E : 0x3043;
}

s16 EnGo2_GetStateGoronCityStairwell(GlobalContext* globalCtx, EnGo2* pthis) {
    if (Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_CLOSING) {
        if (pthis->actor.textId == 0x300E) {
...
u16 EnGo2_GetTextIdGoronMarketBazaar(GlobalContext* globalCtx, EnGo2* pthis) {
    return 0x7122;
}

s16 EnGo2_GetStateGoronMarketBazaar(GlobalContext* globalCtx, EnGo2* pthis) {
    if (Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_CLOSING) {
        return 0;
    } else {
        return 1;
...
u16 EnGo2_GetTextIdGoronCityLostWoods(GlobalContext* globalCtx, EnGo2* pthis) {
    if (!LINK_IS_ADULT) {
...
s16 EnGo2_GetStateGoronCityLostWoods(GlobalContext* globalCtx, EnGo2* pthis) {
    if (Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_CLOSING) {
        if (pthis->actor.textId == 0x3024) {
...
u16 EnGo2_GetTextIdGoronDmtFairyHint(GlobalContext* globalCtx, EnGo2* pthis) {
    if (!LINK_IS_ADULT) {
...
s16 EnGo2_GetStateGoronDmtFairyHint(GlobalContext* globalCtx, EnGo2* pthis) {
    if (Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_CLOSING) {
        return 0;
    } else {
        return 1;
    ...
    if (faceReaction) {
        ...
        switch (pthis->actor.params & 0x1F) {
            case GORON_CITY_ROLLING_BIG:
                return EnGo2_GetTextIdGoronCityRollingBig(globalCtx, pthis);
            case GORON_CITY_LINK:
                return EnGo2_GetTextIdGoronCityLink(globalCtx, pthis);
            case GORON_DMT_BIGGORON:
                return EnGo2_GetTextIdGoronDmtBiggoron(globalCtx, pthis);
            case GORON_FIRE_GENERIC:
                return EnGo2_GetTextIdGoronFireGeneric(globalCtx, pthis);
            case GORON_DMT_BOMB_FLOWER:
                return EnGo2_GetTextIdGoronDmtBombFlower(globalCtx, pthis);
            case GORON_DMT_ROLLING_SMALL:
                return EnGo2_GetTextIdGoronDmtRollingSmall(globalCtx, pthis);
            case GORON_DMT_DC_ENTRANCE:
                return EnGo2_GetTextIdGoronDmtDcEntrance(globalCtx, pthis);
            case GORON_CITY_ENTRANCE:
                return EnGo2_GetTextIdGoronCityEntrance(globalCtx, pthis);
            case GORON_CITY_ISLAND:
                return EnGo2_GetTextIdGoronCityIsland(globalCtx, pthis);
            case GORON_CITY_LOWEST_FLOOR:
                return EnGo2_GetTextIdGoronCityLowestFloor(globalCtx, pthis);
            case GORON_CITY_STAIRWELL:
                return EnGo2_GetTextIdGoronCityStairwell(globalCtx, pthis);
            case GORON_CITY_LOST_WOODS:
                return EnGo2_GetTextIdGoronCityLostWoods(globalCtx, pthis);
            case GORON_DMT_FAIRY_HINT:
                return EnGo2_GetTextIdGoronDmtFairyHint(globalCtx, pthis);
            case GORON_MARKET_BAZAAR:
                return EnGo2_GetTextIdGoronMarketBazaar(globalCtx, pthis);
        }
    }
    return 0;
}
...
s16 EnGo2_GetState(GlobalContext* globalCtx, Actor* thisx) {
    ...
    switch (pthis->actor.params & 0x1F) {
        case GORON_CITY_ROLLING_BIG:
            return EnGo2_GetStateGoronCityRollingBig(globalCtx, pthis);
        case GORON_CITY_LINK:
            return EnGo2_GetStateGoronCityLink(globalCtx, pthis);
        case GORON_DMT_BIGGORON:
            return EnGo2_GetStateGoronDmtBiggoron(globalCtx, pthis);
        case GORON_FIRE_GENERIC:
            return EnGo2_GetStateGoronFireGeneric(globalCtx, pthis);
        case GORON_DMT_BOMB_FLOWER:
            return EnGo2_GetStateGoronDmtBombFlower(globalCtx, pthis);
        case GORON_DMT_ROLLING_SMALL:
            return EnGo2_GetStateGoronDmtRollingSmall(globalCtx, pthis);
        case GORON_DMT_DC_ENTRANCE:
            return EnGo2_GetStateGoronDmtDcEntrance(globalCtx, pthis);
        case GORON_CITY_ENTRANCE:
            return EnGo2_GetStateGoronCityEntrance(globalCtx, pthis);
        case GORON_CITY_ISLAND:
            return EnGo2_GetStateGoronCityIsland(globalCtx, pthis);
        case GORON_CITY_LOWEST_FLOOR:
            return EnGo2_GetStateGoronCityLowestFloor(globalCtx, pthis);
        case GORON_CITY_STAIRWELL:
            return EnGo2_GetStateGoronCityStairwell(globalCtx, pthis);
        case GORON_CITY_LOST_WOODS:
            return EnGo2_GetStateGoronCityLostWoods(globalCtx, pthis);
        case GORON_DMT_FAIRY_HINT:
            return EnGo2_GetStateGoronDmtFairyHint(globalCtx, pthis);
        case GORON_MARKET_BAZAAR:
            return EnGo2_GetStateGoronMarketBazaar(globalCtx, pthis);
    }
    return 0;
}
...
s32 func_80A44790(EnGo2* pthis, GlobalContext* globalCtx) {
    if ((pthis->actor.params & 0x1F) != GORON_DMT_BIGGORON && (pthis->actor.params & 0x1F) != GORON_CITY_ROLLING_BIG) {
        return func_800343CC(globalCtx, &pthis->actor, &pthis->unk_194.unk_00, pthis->unk_218, EnGo2_GetTextId,
                             EnGo2_GetState);
    } else if (((pthis->actor.params & 0x1F) == GORON_DMT_BIGGORON) && ((pthis->collider.base.ocFlags2 & 1) == 0)) {
        return false;
    } else {
        if (Actor_ProcessTalkRequest(&pthis->actor, globalCtx)) {
            pthis->unk_194.unk_00 = 1;
            return true;
        } else if (pthis->unk_194.unk_00 != 0) {
...
void EnGo2_SetColliderDim(EnGo2* pthis) {
    u8 index = pthis->actor.params & 0x1F;

    pthis->collider.dim.radius = D_80A4816C[index].radius;
    pthis->collider.dim.height = D_80A4816C[index].height;
}

void EnGo2_SetShape(EnGo2* pthis) {
    u8 index = pthis->actor.params & 0x1F;
...
void EnGo2_CheckCollision(EnGo2* pthis, GlobalContext* globalCtx) {
    ...
    xzDist = D_80A4816C[pthis->actor.params & 0x1F].xzDist;
    pos.x += (s16)(xzDist * Math_SinS(pthis->actor.shape.rot.y));
    pos.z += (s16)(xzDist * Math_CosS(pthis->actor.shape.rot.y));
    pos.y += D_80A4816C[pthis->actor.params & 0x1F].yDist;
...
s32 func_80A44AB0(EnGo2* pthis, GlobalContext* globalCtx) {
    Player* player = GET_PLAYER(globalCtx);
    f32 arg2;
    ...
    if ((pthis->actor.params & 0x1F) == GORON_DMT_BIGGORON) {
        ...
        if ((pthis->actionFunc != EnGo2_SlowRolling) && (pthis->actionFunc != EnGo2_ReverseRolling) &&
            (pthis->actionFunc != EnGo2_ContinueRolling)) {
            return false;
        } else {
            ...
            if (player->invincibilityTimer <= 0) {
                pthis->collider.base.ocFlags1 |= 8;
            } else {
                return false;
            }
            if (pthis->collider.base.ocFlags2 & 1) {
                ...
                arg2 = pthis->actionFunc == EnGo2_ContinueRolling ? 1.5f : pthis->actor.speedXZ * 1.5f;

                globalCtx->damagePlayer(globalCtx, -4);
                func_8002F71C(globalCtx, &pthis->actor, arg2, pthis->actor.yawTowardsPlayer, 6.0f);
                Audio_PlayActorSound2(&player->actor, NA_SE_PL_BODY_HIT);
                pthis->collider.base.ocFlags1 &= ~0x8;
            }
        }
    }
    return false;
...
s32 EnGo2_UpdateWaypoint(EnGo2* pthis, GlobalContext* globalCtx) {
    s32 change;
    ...
    change = (u8)(pthis->path->count - 1);
    if (pthis->reverse) {
        pthis->waypoint--;
        if (pthis->waypoint < 0) {
            pthis->waypoint = change - 1;
        }
    } else {
        pthis->waypoint++;
        if (pthis->waypoint >= change) {
            pthis->waypoint = 0;
        }
    }
...
s32 EnGo2_Orient(EnGo2* pthis, GlobalContext* globalCtx) {
    s16 targetYaw;
    f32 waypointDistSq = Path_OrientAndGetDistSq(&pthis->actor, pthis->path, pthis->waypoint, &targetYaw);

    Math_SmoothStepToS(&pthis->actor.world.rot.y, targetYaw, 6, 4000, 1);
    if (waypointDistSq > 0.0f && waypointDistSq < SQ(30.0f)) {
        return EnGo2_UpdateWaypoint(pthis, globalCtx);
    } else {
        return 0;
    }
}

s32 func_80A44D84(EnGo2* pthis) {
    s16 targetYaw;

    Path_OrientAndGetDistSq(&pthis->actor, pthis->path, pthis->waypoint, &targetYaw);
    pthis->actor.world.rot.y = targetYaw;
    return 1;
}

s32 EnGo2_IsWakingUp(EnGo2* pthis) {
    s16 yawDiff;
    f32 xyzDist = (pthis->actor.params & 0x1F) == GORON_DMT_BIGGORON ? 800.0f : 200.0f;
    f32 yDist = (pthis->actor.params & 0x1F) == GORON_DMT_BIGGORON ? 400.0f : 60.0f;
    s16 yawDiffAbs;
    ...
    xyzDist = SQ(xyzDist);
    yawDiff = (f32)pthis->actor.yawTowardsPlayer - (f32)pthis->actor.shape.rot.y;
    yawDiffAbs = ABS(yawDiff);
    if (pthis->actor.xyzDistToPlayerSq <= xyzDist && fabsf(pthis->actor.yDistToPlayer) < yDist && yawDiffAbs < 0x2AA8) {
        return true;
    } else {
        return false;
    }
...
s32 EnGo2_IsRollingOnGround(EnGo2* pthis, s16 arg1, f32 arg2, s16 arg3) {
    ...
    if (DECR(pthis->unk_590)) {
        if (!arg3) {
            return true;
        } else {
            pthis->actor.world.pos.y =
                (pthis->unk_590 & 1) ? pthis->actor.world.pos.y + 1.5f : pthis->actor.world.pos.y - 1.5f;
    ...
    if (pthis->unk_59C >= 2) {
        Audio_PlayActorSound2(&pthis->actor, (pthis->actor.params & 0x1F) == GORON_CITY_ROLLING_BIG
    ...
    if (pthis->unk_59C <= 0) {
        if (pthis->unk_59C == 0) {
            pthis->unk_590 = Rand_S16Offset(60, 30);
            pthis->unk_59C = 0;
    ...
    pthis->actor.velocity.y = ((f32)pthis->unk_59C / (f32)arg1) * arg2;
    return true;
}

void EnGo2_BiggoronSetTextId(EnGo2* pthis, GlobalContext* globalCtx, Player* player) {
    u16 textId;

    if ((pthis->actor.params & 0x1F) == GORON_DMT_BIGGORON) {
        if (gSaveContext.bgsFlag) {
            if (func_8002F368(globalCtx) == EXCH_ITEM_CLAIM_CHECK) {
                pthis->actor.textId = 0x3003;
            } else {
                pthis->actor.textId = 0x305E;
            }
...
void func_80A45288(EnGo2* pthis, GlobalContext* globalCtx) {
    Player* player = GET_PLAYER(globalCtx);
    s32 linkAge;
...
void func_80A45360(EnGo2* pthis, f32* alpha) {
    f32 alphaTarget =
        (pthis->skelAnime.animation == &gGoronAnim_004930) && (pthis->skelAnime.curFrame <= 32.0f) ? 0.0f : 255.0f;

    Math_ApproachF(alpha, alphaTarget, 0.4f, 100.0f);
    pthis->actor.shape.shadowAlpha = (u8)(u32)*alpha;
}

void EnGo2_RollForward(EnGo2* pthis) {
    ...
    if (pthis->actionFunc != EnGo2_ContinueRolling) {
...
void func_80A454CC(EnGo2* pthis) {
    switch (pthis->actor.params & 0x1F) {
        case GORON_CITY_ROLLING_BIG:
        case GORON_DMT_DC_ENTRANCE:
        case GORON_CITY_ENTRANCE:
...
f32 EnGo2_GetTargetXZSpeed(EnGo2* pthis) {
    f32 yDist = (pthis->actor.params & 0x1F) == GORON_DMT_BIGGORON ? 400.0f : 60.0f;
    s32 index = pthis->actor.params & 0x1F;

    if (index == GORON_CITY_LINK && (fabsf(pthis->actor.yDistToPlayer) < yDist) &&
        (pthis->actor.xzDistToPlayer < 400.0f)) {
        return 9.0f;
    } else {
...
s32 EnGo2_IsCameraModified(EnGo2* pthis, GlobalContext* globalCtx) {
    ...
    if ((pthis->actor.params & 0x1F) == GORON_DMT_BIGGORON) {
        ...
        } else if (!EnGo2_IsWakingUp(pthis) && (camera->setting == CAM_SET_DIRECTED_YAW)) {
            Camera_ChangeSetting(camera, CAM_SET_DUNGEON1);
            func_8005ACFC(camera, 4);
        }
    }

    if ((pthis->actor.params & 0x1F) == GORON_FIRE_GENERIC || (pthis->actor.params & 0x1F) == GORON_CITY_ROLLING_BIG ||
        (pthis->actor.params & 0x1F) == GORON_CITY_STAIRWELL || (pthis->actor.params & 0x1F) == GORON_DMT_BIGGORON ||
        (pthis->actor.params & 0x1F) == GORON_MARKET_BAZAAR) {
        return true;
...
void EnGo2_WakingUp(EnGo2* pthis) {
    f32 xyzDist = (pthis->actor.params & 0x1F) == GORON_DMT_BIGGORON ? 800.0f : 200.0f;
    s32 isTrue = true;
...
void EnGo2_BiggoronWakingUp(EnGo2* pthis) {
...
void EnGo2_SelectGoronWakingUp(EnGo2* pthis) {
    switch (pthis->actor.params & 0x1F) {
        case GORON_DMT_BOMB_FLOWER:
            pthis->isAwake = true;
...
void EnGo2_EyeMouthTexState(EnGo2* pthis) {
    switch (pthis->eyeMouthTexState) {
        case 1:
            pthis->blinkTimer = 0;
            pthis->eyeTexIndex = 0;
...
void EnGo2_SitDownAnimation(EnGo2* pthis) {
    if ((pthis->skelAnime.playSpeed != 0.0f) && (pthis->skelAnime.animation == &gGoronAnim_004930)) {
        if (pthis->skelAnime.playSpeed > 0.0f && pthis->skelAnime.curFrame == 14.0f) {
            if ((pthis->actor.params & 0x1F) != GORON_DMT_BIGGORON) {
        ...
        if (pthis->skelAnime.playSpeed < 0.0f) {
            if (pthis->skelAnime.curFrame == 1.0f) {
                Audio_PlayActorSound2(&pthis->actor, NA_SE_EN_DODO_M_GND);
            }
            if (pthis->skelAnime.curFrame == 40.0f) {
                Audio_PlayActorSound2(&pthis->actor, NA_SE_EN_GOLON_SIT_DOWN);
            }
        }
    }
...
void EnGo2_GetDustData(EnGo2* pthis, s32 index2) {
    s32 index1 = (pthis->actor.params & 0x1F) == GORON_CITY_ROLLING_BIG ? 1 : 0;
    EnGo2DustEffectData* dustEffectData = &sDustEffectData[index1][index2];

    EnGo2_SpawnDust(pthis, dustEffectData->initialTimer, dustEffectData->scale, dustEffectData->scaleStep,
                    dustEffectData->numDustEffects, dustEffectData->radius, dustEffectData->yAccel);
}

void EnGo2_RollingAnimation(EnGo2* pthis, GlobalContext* globalCtx) {
    if ((pthis->actor.params & 0x1F) == GORON_DMT_BIGGORON) {
        pthis->actor.flags &= ~ACTOR_FLAG_0;
...
void EnGo2_WakeUp(EnGo2* pthis, GlobalContext* globalCtx) {
    if (pthis->skelAnime.playSpeed == 0.0f) {
        if ((pthis->actor.params & 0x1F) != GORON_DMT_BIGGORON) {
...
void EnGo2_SetupRolling(EnGo2* pthis, GlobalContext* globalCtx) {
    if ((pthis->actor.params & 0x1F) == GORON_CITY_ROLLING_BIG || (pthis->actor.params & 0x1F) == GORON_CITY_LINK) {
        pthis->collider.info.bumperFlags = 1;
        pthis->actor.speedXZ = gSaveContext.infTable[17] & 0x4000 ? 6.0f : 3.6000001f;
    } else {
        pthis->actor.speedXZ = 6.0f;
    }
    pthis->actor.flags |= ACTOR_FLAG_24;
    pthis->animTimer = 10;
    pthis->actor.shape.yOffset = 1800.0f;
    pthis->actor.speedXZ += pthis->actor.speedXZ; // Speeding up
    pthis->actionFunc = EnGo2_ContinueRolling;
}
...
void EnGo2_StopRolling(EnGo2* pthis, GlobalContext* globalCtx) {
    ...
    if (((pthis->actor.params & 0x1F) != GORON_CITY_ROLLING_BIG) && ((pthis->actor.params & 0x1F) != GORON_CITY_LINK)) {
        if ((pthis->actor.params & 0x1F) == GORON_DMT_ROLLING_SMALL) {
        ...
        pthis->collider.info.bumperFlags = 0;
    }

    pthis->actor.shape.rot = pthis->actor.world.rot;
    pthis->unk_59C = 0;
    pthis->unk_590 = 0;
    pthis->actionFunc = EnGo2_GroundRolling;
    pthis->actor.shape.yOffset = 0.0f;
    pthis->actor.speedXZ = 0.0f;
}

s32 EnGo2_IsFreeingGoronInFire(EnGo2* pthis, GlobalContext* globalCtx) {
    if ((pthis->actor.params & 0x1F) != GORON_FIRE_GENERIC) {
        return false;
    }
...
s32 EnGo2_IsGoronDmtBombFlower(EnGo2* pthis) {
    if ((pthis->actor.params & 0x1F) != GORON_DMT_BOMB_FLOWER || pthis->unk_194.unk_00 != 2) {
        return false;
    }
...
s32 EnGo2_IsGoronRollingBig(EnGo2* pthis, GlobalContext* globalCtx) {
    if ((pthis->actor.params & 0x1F) != GORON_CITY_ROLLING_BIG || (pthis->unk_194.unk_00 != 2)) {
        return false;
    }
    pthis->unk_194.unk_00 = 0;
    EnGo2_RollingAnimation(pthis, globalCtx);
    pthis->actionFunc = EnGo2_GoronRollingBigContinueRolling;
    return true;
}

s32 EnGo2_IsGoronFireGeneric(EnGo2* pthis) {
    if ((pthis->actor.params & 0x1F) != GORON_FIRE_GENERIC || pthis->unk_194.unk_00 == 0) {
        return false;
    }
    pthis->actionFunc = EnGo2_GoronFireGenericAction;
    return true;
}

s32 EnGo2_IsGoronLinkReversing(EnGo2* pthis) {
    if ((pthis->actor.params & 0x1F) != GORON_CITY_LINK || (pthis->waypoint >= pthis->unk_216) ||
        !EnGo2_IsWakingUp(pthis)) {
        return false;
    }
    return true;
}

s32 EnGo2_IsRolling(EnGo2* pthis) {
    if (pthis->unk_194.unk_00 == 0 || pthis->actor.speedXZ < 1.0f) {
        return false;
    }
    if (EnGo2_IsRollingOnGround(pthis, 2, 20.0 / 3.0f, 0)) {
        if ((pthis->unk_590 >= 9) && (pthis->unk_59C == 0)) {
            pthis->unk_590 = 8;
        }
        EnGo2_GetDustData(pthis, 0);
    }
    return true;
}

void EnGo2_GoronLinkAnimation(EnGo2* pthis, GlobalContext* globalCtx) {
    s32 animation = 13;

    if ((pthis->actor.params & 0x1F) == GORON_CITY_LINK) {
        if ((pthis->actor.textId == 0x3035 && pthis->unk_20C == 0) ||
            (pthis->actor.textId == 0x3036 && pthis->unk_20C == 0)) {
            if (pthis->skelAnime.animation != &gGoronAnim_000D5C) {
                animation = 12;
                pthis->eyeMouthTexState = 0;
            }
        }

        if ((pthis->actor.textId == 0x3032 && pthis->unk_20C == 12) || (pthis->actor.textId == 0x3033) ||
            (pthis->actor.textId == 0x3035 && pthis->unk_20C == 6)) {
            if (pthis->skelAnime.animation != &gGoronAnim_000750) {
                animation = 11;
        ...
        if (pthis->skelAnime.animation == &gGoronAnim_000750) {
            if (pthis->skelAnime.curFrame == 20.0f) {
                Audio_PlayActorSound2(&pthis->actor, NA_SE_EN_GOLON_CRY);
            }
        }
...
void EnGo2_GoronFireCamera(EnGo2* pthis, GlobalContext* globalCtx) {
    s16 yaw;
    ...
    Path_CopyLastPoint(pthis->path, &pthis->at);
    yaw = Math_Vec3f_Yaw(&pthis->actor.world.pos, &pthis->at) + 0xE38;
    pthis->eye.x = Math_SinS(yaw) * 100.0f + pthis->actor.world.pos.x;
    pthis->eye.z = Math_CosS(yaw) * 100.0f + pthis->actor.world.pos.z;
    pthis->eye.y = pthis->actor.world.pos.y + 20.0f;
    pthis->at.x = pthis->actor.world.pos.x;
    pthis->at.y = pthis->actor.world.pos.y + 40.0f;
    pthis->at.z = pthis->actor.world.pos.z;
    Gameplay_CameraSetAtEye(globalCtx, pthis->camId, &pthis->at, &pthis->eye);
}

void EnGo2_GoronFireClearCamera(EnGo2* pthis, GlobalContext* globalCtx) {
    Gameplay_ChangeCameraStatus(globalCtx, MAIN_CAM, CAM_STAT_ACTIVE);
    Gameplay_ClearCamera(globalCtx, pthis->camId);
}

void EnGo2_BiggoronAnimation(EnGo2* pthis) {
    if (INV_CONTENT(ITEM_TRADE_ADULT) >= ITEM_SWORD_BROKEN && INV_CONTENT(ITEM_TRADE_ADULT) <= ITEM_EYEDROPS &&
...
void EnGo2_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    switch (pthis->actor.params & 0x1F) {
        ...
        case GORON_MARKET_BAZAAR:
            ...
            EnGo2_GetItemAnimation(pthis, globalCtx);
            break;
        case GORON_CITY_LINK:
            if ((gSaveContext.infTable[16] & 0x200)) {
                ...
                pthis->collider.dim.height = (D_80A4816C[pthis->actor.params & 0x1F].height * 0.6f);
        ...
        case GORON_DMT_BIGGORON:
            ...
            pthis->collider.base.acFlags = 0;
            pthis->collider.base.ocFlags1 = 0xD; // OC_PLAYER | OC_NO_PUSH | OC_ON
            pthis->actionFunc = EnGo2_CurledUp;
            break;
        case GORON_DMT_BOMB_FLOWER:
...
void EnGo2_CurledUp(EnGo2* pthis, GlobalContext* globalCtx) {
    ...
    if ((s32)pthis->skelAnime.curFrame == 0) {
        ...
        pthis->collider.dim.height =
            ((D_80A4816C[index].height * 0.4f * (pthis->skelAnime.curFrame / pthis->skelAnime.startFrame)) +
             (height * 0.6f));
    }
    if (EnGo2_IsFreeingGoronInFire(pthis, globalCtx)) {
        pthis->isAwake = false;
        EnGo2_WakeUp(pthis, globalCtx);
    }
    if (((pthis->actor.params & 0x1F) != GORON_FIRE_GENERIC) && EnGo2_IsWakingUp(pthis)) {
        EnGo2_WakeUp(pthis, globalCtx);
    }
}
...
void func_80A46B40(EnGo2* pthis, GlobalContext* globalCtx) {
    ...
    if (pthis->unk_211 == true) {
        EnGo2_BiggoronAnimation(pthis);
        EnGo2_GoronLinkAnimation(pthis, globalCtx);
        EnGo2_SelectGoronWakingUp(pthis);

        if (!EnGo2_IsGoronRollingBig(pthis, globalCtx) && !EnGo2_IsGoronFireGeneric(pthis)) {
            if (EnGo2_IsGoronDmtBombFlower(pthis)) {
                return;
            }
        } else {
            return;
        ...
        if (Animation_OnFrame(&pthis->skelAnime, pthis->skelAnime.endFrame)) {
            ...
            height = D_80A4816C[index].height;
            pthis->collider.dim.height =
                (s16)((height * 0.4f * (pthis->skelAnime.curFrame / pthis->skelAnime.endFrame)) + (height * 0.6f));
        }
    }
    if ((!EnGo2_IsCameraModified(pthis, globalCtx)) && (!EnGo2_IsWakingUp(pthis))) {
        EnGo2_RollingAnimation(pthis, globalCtx);
    }
}

void EnGo2_GoronDmtBombFlowerAnimation(EnGo2* pthis, GlobalContext* globalCtx) {
    f32 float1 = pthis->skelAnime.endFrame;
    f32 float2 = pthis->skelAnime.curFrame * ((f32)0x8000 / float1);

    pthis->actor.speedXZ = Math_SinS(float2);
    if ((EnGo2_Orient(pthis, globalCtx)) && (pthis->waypoint == 0)) {
        EnGo2_GetItemAnimation(pthis, globalCtx);
    }
}
...
void EnGo2_ContinueRolling(EnGo2* pthis, GlobalContext* globalCtx) {
    ...
    if (((pthis->actor.params & 0x1F) != GORON_DMT_ROLLING_SMALL || !(pthis->actor.xyzDistToPlayerSq > SQ(float1))) &&
        DECR(pthis->animTimer) == 0) {
        pthis->actionFunc = EnGo2_SlowRolling;
        pthis->actor.speedXZ *= 0.5f; // slowdown
    }
    EnGo2_GetDustData(pthis, 2);
}
...
void EnGo2_SlowRolling(EnGo2* pthis, GlobalContext* globalCtx) {
    ...
    if (!EnGo2_IsRolling(pthis)) {
        if (EnGo2_IsRollingOnGround(pthis, 4, 8.0f, 1) == true) {
            if (EnGo2_IsGoronLinkReversing(pthis)) {
                pthis->actionFunc = EnGo2_ReverseRolling;
                return;
            }
            EnGo2_GetDustData(pthis, 3);
        }
        orientation = EnGo2_Orient(pthis, globalCtx);
        index = pthis->actor.params & 0x1F;
        if (index != GORON_CITY_LINK) {
            if ((index == GORON_DMT_ROLLING_SMALL) && (orientation == 1) && (pthis->waypoint == 0)) {
                EnGo2_StopRolling(pthis, globalCtx);
                return;
            }
        } else if ((orientation == 2) && (pthis->waypoint == 1)) {
            EnGo2_StopRolling(pthis, globalCtx);
            return;
        }
        Math_ApproachF(&pthis->actor.speedXZ, EnGo2_GetTargetXZSpeed(pthis), 0.4f, 0.6f);
        pthis->actor.shape.rot = pthis->actor.world.rot;
    }
}

void EnGo2_GroundRolling(EnGo2* pthis, GlobalContext* globalCtx) {
    if (EnGo2_IsRollingOnGround(pthis, 4, 8.0f, 0)) {
        EnGo2_GetDustData(pthis, 0);
        if (pthis->unk_59C == 0) {
            switch (pthis->actor.params & 0x1F) {
                case GORON_CITY_LINK:
                    pthis->goronState = 0;
                    pthis->actionFunc = EnGo2_GoronLinkStopRolling;
                    break;
                case GORON_CITY_ROLLING_BIG:
                    EnGo2_WakeUp(pthis, globalCtx);
                    break;
...
void EnGo2_ReverseRolling(EnGo2* pthis, GlobalContext* globalCtx) {
    if (!EnGo2_IsRolling(pthis)) {
        Math_ApproachF(&pthis->actor.speedXZ, 0.0f, 0.6f, 0.8f);
        if (pthis->actor.speedXZ >= 1.0f) {
            EnGo2_GetDustData(pthis, 3);
        }
        if ((s32)pthis->actor.speedXZ == 0) {
            pthis->actor.world.rot.y ^= 0x8000;
            pthis->actor.shape.rot.y = pthis->actor.world.rot.y;
            pthis->reverse ^= 1;
            EnGo2_UpdateWaypoint(pthis, globalCtx);
            EnGo2_SetupRolling(pthis, globalCtx);
        }
    }
}
...
void EnGo2_SetupGetItem(EnGo2* pthis, GlobalContext* globalCtx) {
    if (Actor_HasParent(&pthis->actor, globalCtx)) {
        ...
        pthis->actionFunc = EnGo2_SetGetItem;
    } else {
        func_8002F434(&pthis->actor, globalCtx, pthis->getItemId, pthis->actor.xzDistToPlayer + 1.0f,
                      fabsf(pthis->actor.yDistToPlayer) + 1.0f);
    }
}
    ...
    switch (pthis->goronState) {
        case 0: // Wake up
            if (Message_GetState(&globalCtx->msgCtx) == TEXT_STATE_CLOSING) {
                ...
                pthis->unk_194.unk_08 = D_80A4854C;
                pthis->unk_194.unk_0E = D_80A4854C;
                pthis->goronState++;
                pthis->goronState++;
                player->actor.world.rot.y = pthis->actor.world.rot.y;
                player->actor.shape.rot.y = pthis->actor.world.rot.y;
                player->actor.world.pos.x =
                    (f32)((Math_SinS(pthis->actor.world.rot.y) * -30.0f) + pthis->actor.world.pos.x);
                player->actor.world.pos.z =
                    (f32)((Math_CosS(pthis->actor.world.rot.y) * -30.0f) + pthis->actor.world.pos.z);
        ...
        case 2: // Walking away
            if (DECR(pthis->animTimer)) {
                ...
                pthis->animTimer = 0;
                pthis->actor.speedXZ = 0.0f;
                if ((((pthis->actor.params & 0xFC00) >> 0xA) != 1) && (((pthis->actor.params & 0xFC00) >> 0xA) != 2) &&
                    (((pthis->actor.params & 0xFC00) >> 0xA) != 4) && (((pthis->actor.params & 0xFC00) >> 0xA) != 5) &&
                    (((pthis->actor.params & 0xFC00) >> 0xA) != 9) && (((pthis->actor.params & 0xFC00) >> 0xA) != 11)) {
                    pthis->goronState++;
                }
                pthis->goronState++;
            }
        ...
        case 3: // Walking away
            pthis->animTimer++;
            if (!(pthis->animTimer % 8) && (pthis->animTimer < 10)) {
                Audio_PlayActorSound2(&pthis->actor, NA_SE_EN_MORIBLIN_WALK);
            }
            if (pthis->animTimer == 10) {
                Audio_PlayActorSound2(&pthis->actor, NA_SE_EV_IRON_DOOR_OPEN);
            }
            if (pthis->animTimer > 44) {
                Audio_PlaySoundAtPosition(globalCtx, &pthis->actor.world.pos, 20, NA_SE_EV_IRON_DOOR_CLOSE);
            } else {
                break;
            }
        case 4: // Finalize walking away
            Message_CloseTextbox(globalCtx);
            EnGo2_GoronFireClearCamera(pthis, globalCtx);
...
void EnGo2_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Actor_UpdateBgCheckInfo(globalCtx, &pthis->actor, (f32)pthis->collider.dim.height * 0.5f,
                            (f32)pthis->collider.dim.radius * 0.6f, 0.0f, 5);
    if (pthis->unk_194.unk_00 == 0) {
        func_80A44AB0(pthis, globalCtx);
    }
    pthis->actionFunc(pthis, globalCtx);
    if (pthis->unk_211 == true) {
        func_80034F54(globalCtx, pthis->unk_226, pthis->unk_24A, 18);
    }
    func_80A45288(pthis, globalCtx);
    EnGo2_EyeMouthTexState(pthis);
    EnGo2_CheckCollision(pthis, globalCtx);
}

s32 EnGo2_DrawCurledUp(EnGo2* pthis, GlobalContext* globalCtx) {
...
s32 EnGo2_DrawRolling(EnGo2* pthis, GlobalContext* globalCtx) {
    s32 pad;
    ...
    Matrix_RotateZYX((globalCtx->state.frames * ((s16)speedXZ * 1400)), 0, pthis->actor.shape.rot.z, MTXMODE_APPLY);
...
s32 EnGo2_OverrideLimbDraw(GlobalContext* globalCtx, s32 limb, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    ...
    if ((limb == 10) || (limb == 11) || (limb == 14)) {
        float1 = Math_SinS(pthis->unk_226[limb]);
        rot->y += float1 * 200.0f;
        float1 = Math_CosS(pthis->unk_24A[limb]);
        rot->z += float1 * 200.0f;
    }
    return 0;
}
...
void EnGo2_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == 17) {
        Matrix_MultVec3f(&D_80A4856C, &pthis->actor.focus.pos);
    }
}
...
void EnGo2_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    EnGo2_UpdateDust(pthis);
    Matrix_Push();
    EnGo2_DrawDust(pthis, globalCtx);
    Matrix_Pop();

    if ((pthis->actionFunc == EnGo2_CurledUp) && (pthis->skelAnime.playSpeed == 0.0f) &&
        (pthis->skelAnime.curFrame == 0.0f)) {
        if (1) {}
        EnGo2_DrawCurledUp(pthis, globalCtx);
    } else if (pthis->actionFunc == EnGo2_SlowRolling || pthis->actionFunc == EnGo2_ReverseRolling ||
               pthis->actionFunc == EnGo2_ContinueRolling) {
        EnGo2_DrawRolling(pthis, globalCtx);
    } else {
