            ...
            if (globalCtx->sceneNum == SCENE_SPOT15) {
                ...
                if (!(gSaveContext.eventChkInf[1] & 0x10)) {
                    ...
                    if (IS_DAY) {
                        ...
                        pthis->superCuccos[0] = (EnNiw*)Actor_Spawn(
                            &globalCtx->actorCtx, globalCtx, ACTOR_EN_NIW, pthis->actor.world.pos.x + 5.0f,
                            pthis->actor.world.pos.y + 3.0f, pthis->actor.world.pos.z + 26.0f, 0, 0, 0, 0xD);
                        pthis->superCuccos[1] = (EnNiw*)Actor_Spawn(
                            &globalCtx->actorCtx, globalCtx, ACTOR_EN_NIW, pthis->actor.world.pos.x - 20.0f,
                            pthis->actor.world.pos.y + 40.0f, pthis->actor.world.pos.z - 30.0f, 0, 0, 0, 0xD);
                        pthis->superCuccos[2] = (EnNiw*)Actor_Spawn(
                            &globalCtx->actorCtx, globalCtx, ACTOR_EN_NIW, pthis->actor.world.pos.x + 20.0f,
                            pthis->actor.world.pos.y + 40.0f, pthis->actor.world.pos.z - 30.0f, 0, 0, 0, 0xD);
...
s32 func_80B150AC(EnTa* pthis, GlobalContext* globalCtx, s32 idx) {
    ...
    if (player->stateFlags1 & 0x800) {
        ...
        if (interactRangeActor != NULL && interactRangeActor->id == ACTOR_EN_NIW &&
            interactRangeActor == &pthis->superCuccos[idx]->actor) {
            return true;
        }
    }
    return false;
